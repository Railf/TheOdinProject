def caesar_cipher(string, shift_factor)
  ...
  cipher = cipher.map do |character|
    ...
    if lower_case.include?(character)
      character = lower_case[(character - lower_case[0] + shift_factor) % lower_case.size]
    elsif upper_case.include?(character)
      character = upper_case[(character - upper_case[0] + shift_factor) % upper_case.size]
    else
      character
    end
